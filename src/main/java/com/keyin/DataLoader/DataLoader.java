...
public class DataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        loadGateData();
        loadFlightData();
        updateAircraftAirlines(); // because airline can't load before aircraft
    }
    ...
    // Update Aircraft Repository - - - -
    private void updateAircraftAirlines() {
        // List - - - -
        List<Aircraft> updatedAircraftList = new ArrayList<>();
        ...
        for (Aircraft aircraft : aircrafts) {
            if (aircraft.getAirline() != null) {
                long airlineId = aircraft.getAirline().getId();
                Airline airline = airlineRepository.findById(airlineId).orElse(null);
                if (airline != null) {
                    aircraft.setAirline(airline);
                    updatedAircraftList.add(aircraft);
                }
            }
        }

        // Save to Repository - - - -
        aircraftRepository.saveAll(updatedAircraftList);
    }
